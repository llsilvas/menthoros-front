...
import { Reveal, Eyebrow, SectionHeading, CtaButton } from "./primitives";
...
function Section({ id, children, sx }: { id?: string; children: ReactNode; sx?: object }) {
  const t = useTheme();
  return (
    ...
    <Container maxWidth="lg" sx={{ pt: 7, pb: 10 }}>
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1.08fr" }, gap: { xs: 4, md: 6 }, alignItems: "center" }}>
        <Box>
          ...
          <Typography sx={{ fontFamily: mono, color: "text.disabled", fontSize: 12, mt: 2.25, letterSpacing: ".04em" }}>{C.hero.scarcity}</Typography>
        </Box>
    ...
    <Section id="delta">
      ...
      <Reveal>
        <Box sx={{ bgcolor: t.palette.surfaceShift.panel, border: `1px solid ${t.palette.divider}`, borderRadius: "18px", p: 3.5 }}>
          <Typography sx={{ fontFamily: mono, fontSize: 11, letterSpacing: ".14em", color: "text.secondary" }}>{C.delta.context}</Typography>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr auto 1fr" }, gap: 2.25, mt: 2 }}>
            <Box sx={{ bgcolor: "background.paper", border: `1px dashed ${t.palette.divider}`, borderRadius: "14px", p: 2.5 }}>
              <Typography sx={{ fontFamily: mono, fontSize: 11, letterSpacing: ".14em", color: "text.secondary" }}>IA PROPÔS</Typography>
            ...
            <Box sx={{ bgcolor: "background.paper", border: `1px solid ${t.palette.primary.main}`, borderRadius: "14px", p: 2.5 }}>
              <Typography sx={{ fontFamily: mono, fontSize: 11, letterSpacing: ".14em", color: "primary.main" }}>TREINADOR DECIDIU</Typography>
...
export function Fit() {
  ...
  const col = (head: string, items: string[], muted: boolean) => (
    <Box sx={{ bgcolor: "background.paper", border: `1px solid ${t.palette.divider}`, borderRadius: "16px", p: 3.5, height: "100%" }}>
      <Typography sx={{ fontFamily: mono, fontSize: 12, letterSpacing: ".12em", color: muted ? "text.disabled" : "primary.main", mb: 2 }}>{head}</Typography>
      {items.map((x) => (
          ...
          <Box sx={{ bgcolor: "background.paper", border: `1px solid ${t.palette.divider}`, borderRadius: "16px", overflow: "hidden" }}>
            <Box sx={{ position: "relative" }}>
              ...
              <Box sx={{ position: "absolute", left: 24, right: 24, bottom: 20 }}>
                <Typography sx={{ fontFamily: mono, fontSize: 11, letterSpacing: ".14em", color: "primary.main", mb: 0.75 }}>{C.trust.founderLabel}</Typography>
        ...
        <Reveal>
          ...
          <Box sx={{ display: "flex", gap: 1.25, flexWrap: "wrap" }}>
            {C.trust.chips.map((chip) => (
              <Box key={chip} component="span" sx={{ fontFamily: mono, fontSize: 12, color: "text.secondary", border: `1px solid ${t.palette.divider}`, borderRadius: 100, px: 1.6, py: 0.9 }}>{chip}</Box>
            ))}
          </Box>
        </Reveal>
      </Box>
  ...
  return (
    <Box sx={{ bgcolor: "background.paper", border: `1px solid ${t.palette.divider}`, borderRadius: "16px", px: 2.75, mb: 1.5 }}>
      <Box component="button" onClick={() => setOpen((o) => !o)} aria-expanded={open}
        ...
        {q}<Box component="span" sx={{ fontFamily: mono, color: "primary.main", fontSize: 20, flexShrink: 0 }}>{open ? "−" : "+"}</Box>
      </Box>
  ...
  return (
    <Box component="footer" sx={{ borderTop: `1px solid ${t.palette.divider}`, py: 4.5 }}>
      <Container maxWidth="lg" sx={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 2, alignItems: "center" }}>
        ...
        <Typography sx={{ fontFamily: mono, fontSize: 11, color: "text.disabled" }}>© {new Date().getFullYear()} Menthoros · a IA propõe, o treinador decide</Typography>
      </Container>
    </Box>
  );
}
