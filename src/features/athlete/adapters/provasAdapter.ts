 ...
 * `diasFaltando` vem direto do DTO (já calculado pelo backend, D0.3) — não recalcula a diferença
 * de dias no frontend. Retorna `null` quando não há prova futura cadastrada (CA2 — CTA honesto).
 */
export function buildProximaProva(provas: Prova[], hoje: Date = new Date()): ProximaProva | null {
  ...
  return { nomeProva: proxima.nomeProva, diasFaltando: proxima.diasFaltando ?? 0 };
}
