...
import { Box, Button, Link, Paper, Stack, Typography, Table, TableBody, TableCell, TableContainer, TableHead, TableRow } from '@mui/material';
import { gradients, glassAzulSx, surface } from '../../theme/tokens';
import { overlayWhite } from '../../theme/overlays';

const CONTATO_EMAIL = 'contato@menthoros.com';
const ATUALIZADA_EM = '2 de agosto de 2026';
...
const SECOES: Secao[] = [
  {
    ...
    paragrafos: [
      ...
      `Encarregado de Proteção de Dados (DPO): Leandro Alves da Silva — ${CONTATO_EMAIL}`,
    ],
  },
  {
    titulo: '2. Dados pessoais coletados',
    paragrafos: ['A Menthoros coleta os dados estritamente necessários à prestação dos serviços, classificados conforme segue.'],
  },
  {
    titulo: '2.1 Dados fornecidos diretamente pelo usuário',
    paragrafos: [
  ...
  {
    titulo: '2.2 Dados recebidos de integrações com terceiros',
    paragrafos: [
  ...
  {
    titulo: '2.3 Dados coletados automaticamente',
    paragrafos: [
      'Endereço IP, identificadores de dispositivo, sistema operacional, tipo de navegador e dados de log de acesso. Cookies e tecnologias similares, conforme Seção 8.',
    ],
  },
  {
  ...
  {
    titulo: '3.1 Inteligência Artificial',
    paragrafos: [
      ...
      'Durante a fase atual de MVP, o processamento por IA utiliza contas de desenvolvedor. A Menthoros se compromete a formalizar Data Processing Agreements (DPAs) com ambos os provedores antes do lançamento comercial, incluindo cláusulas contratuais padrão (SCCs) para transferência internacional conforme Art. 33 da LGPD.',
  ...
  },
  {
    ...
    paragrafos: [
      'O tratamento dos dados pessoais pela Menthoros fundamenta-se nas seguintes bases legais:',
    ],
    tabela: {
      cabecalho: ['Finalidade', 'Base legal', 'Consentimento revogável?'],
      linhas: [
        ['Dados cadastrais (nome, e-mail, data nasc.)', 'Execução de contrato (Art. 7º, V)', 'Não — necessário ao funcionamento'],
        ['Dados de treino (distância, pace, duração)', 'Execução de contrato (Art. 7º, V)', 'Não — necessário ao funcionamento'],
        ['Sexo', 'Consentimento explícito (Art. 11, I)', 'Sim — com impacto em zonas de FC/VO2max'],
        ['Dados de saúde (lesões, check-in, FC, GPS)', 'Consentimento explícito (Art. 11, I)', 'Sim — funcionalidades relacionadas são desabilitadas'],
        ['Geração de planos por IA (OpenAI)', 'Consentimento (Art. 7º, I)', 'Sim — planos passam a ser manuais'],
        ['Análise pós-treino por IA (Anthropic)', 'Consentimento (Art. 7º, I)', 'Sim — análises automáticas são desabilitadas'],
        ['Integrações (Strava, Garmin, etc.)', 'Consentimento — autorizado na plataforma de origem', 'Sim — desconecte a integração'],
        ['Convite de atleta (envio de e-mail)', 'Legítimo interesse (Art. 7º, IX)', 'O convidado pode solicitar remoção'],
        ['Dados do treinador', 'Execução de contrato (Art. 7º, V)', 'Não — necessário ao funcionamento'],
        ['Dados da assessoria (razão social, CNPJ)', 'Execução de contrato (Art. 7º, V)', 'Não — necessário à prestação do serviço'],
      ],
    },
  },
  {
  ...
  {
    titulo: '6.1 Com o treinador e assessoria',
    paragrafos: [
  ...
  {
    titulo: '6.2 Com provedores de infraestrutura e subprocessadores',
    tabela: {
      cabecalho: ['Provedor', 'Serviço', 'Dados', 'País', 'DPA'],
      linhas: [
        ['AWS', 'Hospedagem e banco de dados (PostgreSQL RDS)', 'Todos os dados da plataforma', 'Brasil (sa-east-1)', 'A ser formalizado'],
        ['OpenAI', 'IA — geração de planos', 'Perfil, histórico, métricas, provas, lesões', 'EUA', 'A ser formalizado'],
        ['Anthropic', 'IA — análise pós-treino', 'Dados do treino, métricas, FC', 'EUA', 'A ser formalizado'],
        ['Keycloak', 'Autenticação e organizações', 'Nome, e-mail, grupo/tenant', 'Brasil (self-hosted)', 'N/A — infra própria'],
        ['Cloudflare', 'DNS e recebimento de e-mail', 'E-mails recebidos no domínio', 'Global (CDN)', 'N/A — infra de domínio'],
      ],
    },
  },
  {
    titulo: '6.3 Não comercializamos dados',
    paragrafos: [
  ...
  },
  {
    ...
    paragrafos: [
      'Os dados enviados à OpenAI e Anthropic são processados em servidores nos Estados Unidos. Ambos os provedores mantêm certificações de segurança compatíveis com a legislação brasileira (ISO 27001, SOC 2 Type II) e não utilizam dados de API para treinamento de modelos — essa política se aplica tanto a contas empresariais quanto a contas de desenvolvedor. Antes do lançamento comercial, a Menthoros formalizará Data Processing Agreements (DPAs) incluindo Standard Contractual Clauses (SCCs) conforme Art. 33, IX da LGPD.',
    ],
  },
  {
  ...
  },
  {
    ...
    paragrafos: [
      'Nos termos do Art. 18 da LGPD, o titular pode, a qualquer momento e gratuitamente:',
    ],
    tabela: {
      cabecalho: ['Direito', 'Como exercer', 'Prazo'],
      linhas: [
        ['Confirmação e acesso aos dados', 'Solicite por e-mail ou visualize no aplicativo', 'Até 15 dias'],
        ['Correção de dados incompletos', 'Edite no seu perfil (self-service)', 'Imediato'],
        ['Anonimização, bloqueio ou eliminação', 'Solicite por e-mail', 'Até 15 dias'],
        ['Portabilidade para outro serviço', 'Solicite exportação em JSON/CSV', 'Até 15 dias'],
        ['Revogação do consentimento', 'Configurações > Privacidade', 'Imediato; efeitos em até 24h'],
  ...
  },
  {
    ...
    tabela: {
      cabecalho: ['Dado', 'Período de retenção', 'Justificativa'],
      linhas: [
        ['Dados de perfil e treino', 'Atleta ativo + 30 dias após exclusão', 'Conclusão de faturamento e backups'],
        ['Dados de check-in', 'Últimos 12 meses (rolagem automática)', 'Janela clínica para análise de carga'],
        ['Atividades importadas', 'Integração ativa + 30 dias após desconexão', 'Permite reimportação'],
        ['Dados do treinador e assessoria', 'Assessoria ativa + 90 dias', 'Encerramento contratual B2B'],
        ['Convites não aceitos', '7 dias após envio', 'Legítimo interesse; remoção imediata se solicitado'],
        ['Backups', '30 dias', 'Criptografados (AES-256); sobrescritos no ciclo normal'],
      ],
    },
  },
  {
  ...
  },
  {
    ...
    paragrafos: [
      'A Menthoros adota medidas técnicas e administrativas de segurança compatíveis com as melhores práticas de mercado:',
    ],
    itens: [
      'Criptografia em trânsito (TLS 1.3)',
      'Criptografia em repouso (AES-256)',
      'Isolamento de dados por assessoria (tenant_id em todas as tabelas + TenantInterceptor)',
  ...
  {
    titulo: '11.1 Incidentes de segurança',
    paragrafos: [
  ...
  {
    titulo: '11.2 RIPD (Relatório de Impacto à Proteção de Dados)',
    paragrafos: [
      'O Menthoros mantém Relatório de Impacto à Proteção de Dados Pessoais (RIPD) para os tratamentos de dados sensíveis em larga escala, conforme Art. 38 da LGPD e Resolução CD/ANPD nº 2/2022. Elaborado em 31 de julho de 2026. Disponível para consulta mediante solicitação ao DPO.',
    ],
  },
  {
    titulo: '12. Idade mínima',
    paragrafos: [
      'A plataforma Menthoros é destinada a usuários com 18 anos ou mais. Para atletas entre 16 e 18 anos, o cadastro deve ser realizado pelo responsável legal, que fornecerá o consentimento parental conforme Art. 14 da LGPD. Não realizamos tratamento de dados de crianças menores de 16 anos. A Menthoros poderá solicitar, a qualquer tempo, comprovação da autorização dos pais ou responsável legal, podendo suspender o cadastro na ausência de tal comprovação.',
    ],
  },
  {
...
export default function PrivacidadePage() {
  return (
    ...
    >
      <Paper elevation={0} sx={{ width: '100%', maxWidth: 820, my: { xs: 2, sm: 5 }, p: { xs: 3, sm: 5 }, borderRadius: 2, ...glassAzulSx }}>
        ...
        <Typography variant="body2" sx={{ color: overlayWhite[70], mb: 3 }}>
          Esta política explica como a Menthoros coleta, utiliza, armazena, compartilha e protege os dados pessoais dos usuários
          da plataforma Menthoros, em conformidade com a Lei Geral de Proteção de Dados (Lei nº 13.709/2018 — LGPD).
          Aplica-se a todos os usuários — atletas, treinadores e assessorias esportivas.
        </Typography>

        <Stack spacing={3}>
          {SECOES.map((secao, idx) => (
            <Box key={`${secao.titulo}-${idx}`}>
              {secao.titulo ? (
                <Typography variant={secao.titulo.includes('.') ? 'subtitle1' : 'h6'} sx={{ fontWeight: 700, color: surface[0], mb: 1 }}>
                  {secao.titulo}
                </Typography>
              ) : null}
                  ...
                  <Table size="small">
                    <TableHead>
                      <TableRow>
                        {secao.tabela.cabecalho.map((h) => (
                          <TableCell key={h} sx={{ color: surface[0], fontWeight: 700, borderColor: 'rgba(255,255,255,0.1)', py: 1 }}>
                            {h}
                          </TableCell>
                        ))}
                      </TableRow>
                    ...
                    <TableBody>
                      {secao.tabela.linhas.map((linha, li) => (
                        <TableRow key={li}>
                          {linha.map((cel, ci) => (
                            <TableCell key={ci} sx={{ color: overlayWhite[70], borderColor: 'rgba(255,255,255,0.06)', py: 1 }}>
                              {cel}
                            </TableCell>
                          ))}
                        </TableRow>
        ...
        <Box sx={{ mt: 4, pt: 3, borderTop: 1, borderColor: 'divider' }}>
          <Typography variant="body2" sx={{ color: overlayWhite[70] }}>
            Dúvidas? Fale com o Encarregado de Proteção de Dados (DPO) em{' '}
